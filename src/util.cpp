...
const char * const BITCOIN_CONF_FILENAME = "csignal.conf";
const char * const BITCOIN_PID_FILENAME = "csignald.pid";
 ...
 * by well-meaning csignal adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "csignal" is a composite category enabling all CSIGNAL-related debug output
            if(ptrCategory->count(string("csignal"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CSIGNALCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CSIGNALCore
    // Mac: ~/Library/Application Support/CSIGNALCore
    // Unix: ~/.csignalcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CSIGNALCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CSIGNALCore";
#else
    // Unix
    return pathRet / ".csignalcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty csignal.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override csignal.conf
