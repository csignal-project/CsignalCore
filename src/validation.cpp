...
#if defined(NDEBUG)
# error "CSIGNAL Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    }  else if (nPrevHeight <= 1200 && nPrevHeight >= 2) {
      nSubsidy = 1 * COIN;
    } else if (nPrevHeight <= 5000 && nPrevHeight >= 1201) {
      nSubsidy = 3 * COIN;
    } else if (nPrevHeight <= 10000 && nPrevHeight >= 5001) {
      nSubsidy = 5 * COIN;
    } else if (nPrevHeight <= 11200 && nPrevHeight >= 10001) {
      nSubsidy = 8 * COIN;
    } else if (nPrevHeight <= 15000 && nPrevHeight >= 11201) {
      nSubsidy = 12 * COIN;
    } else if (nPrevHeight <= 20000 && nPrevHeight >= 15001) {
      nSubsidy = 9 * COIN;
    } else if (nPrevHeight <= 22000 && nPrevHeight >= 20001) {
      nSubsidy = 6 * COIN;
    } else if (nPrevHeight <= 25000 && nPrevHeight >= 22001) {
      nSubsidy = 7 * COIN;
    } else if (nPrevHeight <= 35000 && nPrevHeight >= 25001) {
      nSubsidy = 10 * COIN;
    } else if (nPrevHeight <= 48000 && nPrevHeight >= 35001) {
      nSubsidy = 15 * COIN;
    } else if (nPrevHeight <= 50000 && nPrevHeight >= 48001) {
      nSubsidy = 11 * COIN;
    } else if (nPrevHeight <= 75000 && nPrevHeight >= 50001) {
      nSubsidy = 8 * COIN;
    } else if (nPrevHeight <= 100000 && nPrevHeight >= 75001) {
      nSubsidy = 6 * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // CSIGNAL : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CSIGNAL): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CSIGNAL): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CSIGNAL : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CSIGNAL): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CSIGNAL): spork is off, skipping transaction locking checks\n");
    }
