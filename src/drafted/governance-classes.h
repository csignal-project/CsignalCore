 ...
 *
 *   // network
 *   CCSIGNALNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.csignal.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CCSIGNALNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CCSIGNALNetwork(UniValue objIn)
    {
...
// // can be under: CSIGNALNetwork
// //   -- signature requirements : Key1(User)
// class CCSIGNALNetworkVariable : public CGovernanceObject
// {
// private:
